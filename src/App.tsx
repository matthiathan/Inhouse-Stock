...
const WarehousePage = lazyNamed(() => import('./pages/warehouse'), 'WarehousePage');
const AssetsPage = lazyNamed(() => import('./pages'), 'AssetsPage');
const NewAssetPage = lazyNamed(() => import('./pages/NewAssetPage'), 'NewAssetPage');
const ScannerPage = lazyNamed(() => import('./pages'), 'ScannerPage');
const SettingsPage = lazyNamed(() => import('./pages'), 'SettingsPage');
const AssetDetailsPage = lazyNamed(() => import('./pages/AssetDetailsPage'), 'AssetDetailsPage');
const CustomerDetailsPage = lazyNamed(() => import('./pages'), 'CustomerDetailsPage');
...
const OrderFulfillmentPage = lazyNamed(() => import('./pages/OrderFulfillmentPage'), 'OrderFulfillmentPage');
const TechRoutePage = lazyNamed(() => import('./pages/TechRoutePage'), 'TechRoutePage');
const TechTicketPage = lazy(() => import('./pages/TechTicketPage'));
const ServiceTasksPage = lazy(() => import('./pages/ServiceTasksPage'));
const DispatchRoutingHub = lazyNamed(() => import('./pages/DispatchRoutingHub'), 'DispatchRoutingHub');
...
function RoleProtectedRoute({ children, allowedRoles }: RoleProtectedRouteProps) {
  ...
  if (!allowedRoles.includes(userRole)) {
    const redirectTarget = (userRole === 'tech' || userRole === 'road_tech' || userRole === 'driver' || userRole === 'user') ? '/scanner' : '/warehouse';
...
function IndexRedirect() {
  ...
  const target = (normalizedRole === 'tech' || normalizedRole === 'road_tech' || normalizedRole === 'driver')
    ? '/my-route'
    : (normalizedRole === 'user' ? '/scanner' : '/warehouse');
  return <Navigate to={target} replace />;
}

function RouteFallback() {
  return (
    <div className="flex min-h-[320px] items-center justify-center bg-bg-base font-mono text-brand-gold">
          ...
          <Routes>
            ...
            >
              ...
              <Route
                path="warehouse"
                element={
                  <RoleProtectedRoute allowedRoles={['admin', 'ops_manager', 'warehouse', 'warehouse_staff']}>
                    <WarehousePage />
                  </RoleProtectedRoute>
                }
              />
              ...
              <Route
                path="fulfillment"
                element={
                  <RoleProtectedRoute allowedRoles={['warehouse', 'warehouse_staff']}>
              ...
              <Route
                path="my-route"
                element={
                  <RoleProtectedRoute allowedRoles={['tech', 'road_tech', 'driver']}>
                    <TechRoutePage />
                  </RoleProtectedRoute>
                }
              />
